...
if (isset($_POST['submit']) && is_numeric($_GET['id'])) {
  ...
  foreach ($_POST as $key => $value) {    
    if ($key!= 'submit' && $key != 'fileToUpload' && $key != "old_file_name") {
      ...
      $sql = "UPDATE `o_our_work` SET `$key` = '$value' WHERE `id` = '$id'";
  ...
  if ($_FILES['fileToUpload']['tmp_name'] != NULL) {
    ...
    $sql = "UPDATE `o_our_work` SET `bg` = '$img_name' WHERE `id` = '$id'";
      $result = $conn->query($sql);
  }
  if ($result) {
...
if (isset($_POST['save']) && is_numeric($_GET['id'])) {
  ...
  $sql = "INSERT INTO `o_our_work` (`class`, `bg`, `link`, `title`, `category`) VALUES ('$class', '$img_name', '$link', '$title', '$category')";
  $result = $conn->query($sql);
  if ($result) {
...
<head>
  ...
  <?php include('header.php'); ?>
    <div class="panel panel-default">
      <div class="panel-heading">Ongoing Project updater</div>
    ...
    <span class="text-danger"><?php echo $message; if (isset($_GET['error'])) {
        ...
        <form action="" method="POST" enctype="multipart/form-data">
          ...
          <?php
              ...
              $sql = "SELECT * FROM `o_our_work` WHERE `id` = '$id'";
              $result = $conn->query($sql);
              ...
              if ($row) {
                echo
                  ...
                  <td><img height='80px' src='../img/bg/".$row['bg']."'><input type='hidden' name='old_file_name' value='".get_item_details('bg',$id,'o_our_work')."'><input type='file' name='fileToUpload' id='fileToUpload'></td> 
