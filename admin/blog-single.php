...
if (isset($_POST['submit']) && is_numeric($_GET['id'])) {
  ...
  foreach ($_POST as $key => $value) {    
    if ($key!= 'submit' && $key != 'fileToUpload' && $key != "old_file_name") {
      ...
      $sql = "UPDATE `blog` SET `$key` = '$value' WHERE `id` = '$id'";
  ...
  if ($_FILES["fileToUpload"]["size"]) {
    ...
    $sql = "UPDATE `blog` SET `blog_cover` = '$img_name' WHERE `id` = '$id'";
      $result = $conn->query($sql);
  }
  if ($result) {
...
if (isset($_POST['save']) && is_numeric($_GET['id'])) {
  ...
  $sql = "INSERT INTO `blog` (`blog_cover`, `title`, `description`, `html`, `date`) VALUES ('$img_name', '$title', '$description', '$html', '$date')";
  $result = $conn->query($sql);
  if ($result) {
...
<html>
<head>
  <title>Green Castleae | Admin Dashboard</title>
  <style>
  table {
  border-spacing: 10;
  width: 100%;
          ...
          <?php
              ...
              if ($row) {
                echo
                  ...
                  <td><img height='80px' src='../img/bg/".$row['blog_cover']."'><input type='hidden' name='old_file_name' value='".get_item_details('blog_cover',$id,'blog')."'><input type='file' name='fileToUpload' id='fileToUpload'></td>
                  <td><textarea name='description'>".$row['description']."</textarea></td>
                  </tr>
                  <tr><center><td colspan='3'>Content<textarea name='html'>".get_item_details('html',$id,'blog')."</textarea>
                  <script>
                ...
                "<tr>
                  ...
                  <tr><center><td colspan='3'>Content<textarea name='html'></textarea>
                  <script>
                      CKEDITOR.replace('html');
                  </script><center>
                  </td>
