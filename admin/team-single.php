...
if (isset($_POST['submit']) && is_numeric($_GET['id'])) {
  ...
  if ($_FILES['fileToUpload']['tmp_name'] != NULL) {
    ...
    $sql = "UPDATE `team` SET `bg` = '$img_name' WHERE `id` = '$id'";
      $result = $conn->query($sql);
  }
  if ($result) {
...
if (isset($_POST['save']) && is_numeric($_GET['id'])) {
  ...
  $sql = "INSERT INTO `team` (`bg`, `icon`, `title`, `description`, `html`) VALUES ('$img_name', '$icon', '$title', '$description', '$html')";
  $result = $conn->query($sql);
  if ($result) {
...
<head>
  ...
  <?php include('header.php'); ?>
    <div class="panel panel-default">
      <div class="panel-heading">team Updater</div>
          ...
          <?php
              ...
              if ($row) {
                echo
                  ...
                  <td><img height='80px' src='../img/bg/".$row['bg']."'><input type='hidden' name='old_file_name' value='".get_item_details('bg',$id,'team')."'><input type='file' name='fileToUpload' id='fileToUpload'></td>
